...
def  main():
  ...
  installer = "\installer\IPlugEmbedSwift Installer.exe"
   
  if demo:
    installer = "\installer\IPlugEmbedSwift Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugEmbedSwift manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugEmbedSwift-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugEmbedSwift-v" + FULLVERSIONSTR + "-win-demo.zip"
